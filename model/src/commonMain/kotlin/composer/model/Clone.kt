...
fun Node.cloneWithNewIds(newId: () -> String): Node = when (this) {
    ...
    is Node.Line -> copy(id = newId())
    is Node.RectShape -> copy(id = newId())
    is Node.CircleShape -> copy(id = newId())
    is Node.EllipseShape -> copy(id = newId())
    is Node.ArcShape -> copy(id = newId())
    is Node.Canvas -> copy(id = newId(), children = children.map { it.cloneWithNewIds(newId) })
    is Node.TabRow -> copy(id = newId(), children = children.map { it.cloneWithNewIds(newId) })
    is Node.NavigationBar -> copy(id = newId(), children = children.map { it.cloneWithNewIds(newId) })
    is Node.BadgedBox -> copy(id = newId(), children = children.map { it.cloneWithNewIds(newId) })
